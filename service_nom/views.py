...
from service_nom.pagination import ResultsSetPagination
from service_nom.authentication import AdminAuthentication
from service_nom.serializers import (
    ActivateSerializer, DeactivateSerializer, AdminCompanySerializer,
    CurrencySerializer
)
from service_nom.models import Currency
...
def root(request, format=None):
    return Response(
        [
            {'Public': OrderedDict([
                ('Activate', reverse('service_nom:activate',
                    request=request,
                    format=format)),
                ('Deactivate', reverse('service_nom:deactivate',
                    request=request,
                    format=format))
            ])},
            {'Admins': OrderedDict([
                ('Company', reverse('service_nom:admin-company',
                    request=request,
                    format=format)),
                ('Currencies', reverse('service_nom:admin-currencies',
                    request=request,
                    format=format))
            ])},
        ])
